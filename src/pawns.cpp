...
namespace {
  ...
  constexpr Score PawnsStorm    = S(25, 25);
...
Score Entry::evaluate_shelter(const Position& pos, Square ksq) {
  ...
  for (File f = File(center - 1); f <= File(center + 1); ++f)
  {
      ...
      if (ourRank && (ourRank == theirRank - 1))
          bonus -= BlockedStorm * int(theirRank == RANK_3), blockedStorm += int(theirRank == RANK_3);
      else
          bonus -= make_score(UnblockedStorm[d][theirRank], 0), unblockedStorm += bool(theirRank);
  }

  bonus -= PawnsStorm * (blockedStorm * unblockedStorm);
