...
namespace {
  ...
  enum Stages {
    MAIN_SEARCH, CAPTURES_INIT, GOOD_CAPTURES, KILLERS, COUNTERMOVE1, COUNTERMOVE2, QUIET_INIT, QUIET, BAD_CAPTURES,
...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const ButterflyHistory* mh,
                       const CapturePieceToHistory* cph, const PieceToHistory** ch, const std::array<Move,2>& cm, Move* killers_p)
           : pos(p), mainHistory(mh), captureHistory(cph), contHistory(ch), countermoves{cm[0], cm[1]},
...
Move MovePicker::next_move(bool skipQuiets) {
  ...
  case COUNTERMOVE1:
      ...
      if (    move != MOVE_NONE
          ...
          && !pos.capture(move))
      {
          ++stage; // skip second counter move
          return move;
      }
      /* fallthrough */
    ...
    case COUNTERMOVE2:
      ++stage;
      move = countermoves[0] = countermoves[1];
      if (    move != MOVE_NONE
          &&  move != ttMove
          &&  move != killers[0]
          &&  move != killers[1]
