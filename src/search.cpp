...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool skipEarlyPruning) {
    ...
    bool captureOrPromotion, doFullDepthSearch, moveCountPruning, bestMoveCountPruning, skipQuiets, ttCapture;
    Piece movedPiece;
...
moves_loop: // When in check search starts from here
    ...
    while ((move = mp.next_move(skipQuiets)) != MOVE_NONE)
    {
      ...
      if (value > bestValue)
      {
          ...
          {
              bestMove = move;
              bestMoveCountPruning = moveCountPruning;
    ...
    */
    ...
    else if (bestMove)
    {
        ...
        if (!pos.capture_or_promotion(bestMove))
            update_stats(pos, ss, bestMove, quietsSearched, quietCount, stat_bonus(depth + (bestMoveCountPruning && depth < 5 * ONE_PLY) * 2 * ONE_PLY));
