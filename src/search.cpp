        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (!pos.capture_or_promotion(ttMove))
                    update_quiet_stats(pos, ss, ttMove, stat_bonus(depth), depth, improving);
...
moves_loop: // When in check, search starts from here
    ...
    Move countermove = thisThread->counterMoves[PvNode][pos.piece_on(prevSq)][prevSq];
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth, bool improving) {
    ...
    {
        Square prevSq = to_sq((ss-1)->currentMove);
        thisThread->counterMoves[improving][pos.piece_on(prevSq)][prevSq] = move;

        if (!thisThread->counterMoves[!improving][pos.piece_on(prevSq)][prevSq])
            thisThread->counterMoves[!improving][pos.piece_on(prevSq)][prevSq] = move;
    }
