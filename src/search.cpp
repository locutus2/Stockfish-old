...
moves_loop: // When in check, search starts from here
    ...
    Move countermove = thisThread->counterMoves[(ss-1)->moveCount == 1][pos.piece_on(prevSq)][prevSq];
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth) {
    ...
    {
        Square prevSq = to_sq((ss-1)->currentMove);
        thisThread->counterMoves[(ss-1)->moveCount == 1][pos.piece_on(prevSq)][prevSq] = move;

        if (!thisThread->counterMoves[(ss-1)->moveCount != 1][pos.piece_on(prevSq)][prevSq])
            thisThread->counterMoves[(ss-1)->moveCount != 1][pos.piece_on(prevSq)][prevSq] = move;
    }
