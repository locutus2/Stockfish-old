...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    bool formerPv, givesCheck, improving, didLMR, priorCaptureOrQueenPromotion;
    ...
    priorCaptureOrQueenPromotion = pos.captured_piece() || promotion_type((ss-1)->currentMove) == QUEEN;
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if ((ss-1)->moveCount <= 2 && !priorCaptureOrQueenPromotion)
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCaptureOrQueenPromotion)
    {
