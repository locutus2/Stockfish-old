...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    tte = TT.probe(posKey, us, ss->ttHit);
    ...
    if (!rootNode && TB::Cardinality)
    {
            ...
            && !pos.can_castle(ANY_CASTLING))
        {
            ...
            if (err != TB::ProbeState::FAIL)
            {
                ...
                {
                    tte->save(posKey, value_to_tt(value, ss->ply), us, ss->ttPv, b,
    ...
    else
    {
        ...
        if (!excludedMove)
            tte->save(posKey, VALUE_NONE, us, ss->ttPv, BOUND_NONE, DEPTH_NONE, MOVE_NONE, eval);
    }
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= probCutBeta)
                {
                    ...
                    if ( !(ss->ttHit
                       && tte->depth() >= depth - 3
                       && ttValue != VALUE_NONE))
                        tte->save(posKey, value_to_tt(value, ss->ply), us, ttPv,
                            BOUND_LOWER,
...
moves_loop: // When in check, search starts here
    ...
    */
    ...
    if (!excludedMove && !(rootNode && thisThread->pvIdx))
        tte->save(posKey, value_to_tt(bestValue, ss->ply), us, ss->ttPv,
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    posKey = pos.key();
    tte = TT.probe(posKey, pos.side_to_move(), ss->ttHit);
    ttValue = ss->ttHit ? value_from_tt(tte->value(), ss->ply, pos.rule50_count()) : VALUE_NONE;
    ...
    else
    {
        ...
        if (bestValue >= beta)
        {
            ...
            if (!ss->ttHit)
                tte->save(posKey, value_to_tt(bestValue, ss->ply), pos.side_to_move(), false, BOUND_LOWER,
    ...
    // Save gathered info in transposition table
    tte->save(posKey, value_to_tt(bestValue, ss->ply), pos.side_to_move(), pvHit,
...
bool RootMove::extract_ponder_from_tt(Position& pos) {
    ...
    pos.do_move(pv[0], st);
    TTEntry* tte = TT.probe(pos.key(), pos.side_to_move(), ttHit);
