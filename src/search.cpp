...
namespace {
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, bool givesCheck, int bonus);
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if ((ss-1)->moveCount <= 2 && !priorCapture)
                    update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, ss->inCheck, -stat_bonus(depth + 1));
            }
            ...
            else if (!ttCapture)
            {
                ...
                update_continuation_histories(ss, pos.moved_piece(ttMove), to_sq(ttMove), pos.gives_check(ttMove), penalty);
            }
        }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          {
              int history =   (*contHist[0])[movedPiece][to_sq(move)][givesCheck]
                            + (*contHist[1])[movedPiece][to_sq(move)][givesCheck]
                            + (*contHist[3])[movedPiece][to_sq(move)][givesCheck];
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
          ...
          // Quiet ttMove extensions (~0 Elo)
                   ...
                   && move == ttMove
                   && move == ss->killers[0]
                   && (*contHist[0])[movedPiece][to_sq(move)][givesCheck] >= 5491)
              extension = 1;
      }
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          ss->statScore =  thisThread->mainHistory[us][from_to(move)]
                         + (*contHist[0])[movedPiece][to_sq(move)][givesCheck]
                         + (*contHist[1])[movedPiece][to_sq(move)][givesCheck]
                         + (*contHist[3])[movedPiece][to_sq(move)][givesCheck]
                         - 4334;
      ...
      if (doFullDepthSearch)
      {
          ...
          if (didLMR)
          {
              ...
              update_continuation_histories(ss, movedPiece, to_sq(move), givesCheck, bonus);
          }
      }
    ...
    */
             ...
             && !priorCapture)
    {
        ...
        update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, ss->inCheck, stat_bonus(depth) * (1 + extraBonus));
    }
  ...
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
      ...
      if (  !capture
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY
          && (*contHist[0])[pos.moved_piece(move)][to_sq(move)][givesCheck] < CounterMovePruneThreshold
          && (*contHist[1])[pos.moved_piece(move)][to_sq(move)][givesCheck] < CounterMovePruneThreshold)
          continue;
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            update_continuation_histories(ss, pos.moved_piece(quietsSearched[i]), to_sq(quietsSearched[i]), pos.gives_check(quietsSearched[i]), -bonus2);
        }
    }
    else
    ...
    if (   ((ss-1)->moveCount == 1 + (ss-1)->ttHit || ((ss-1)->currentMove == (ss-1)->killers[0]))
        && !pos.captured_piece())
            update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, ss->inCheck, -bonus1);
  ...
  void update_continuation_histories(Stack* ss, Piece pc, Square to, bool givesCheck, int bonus) {
    ...
    for (int i : {1, 2, 4, 6})
    {
        ...
        if (is_ok((ss-i)->currentMove))
            (*(ss-i)->continuationHistory)[pc][to][givesCheck] << bonus;
    }
  }
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus) {
    ...
    update_continuation_histories(ss, pos.moved_piece(move), to_sq(move), pos.gives_check(move), bonus);
