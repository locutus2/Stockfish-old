...
namespace {
  ...
  int stat_bonus(Depth d, Stack* ss) {
    return std::min(((9 * d + 270) * d - 311) * (2 - ss->ttHit), 2145);
  }
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (!ttCapture)
                    update_quiet_stats(pos, ss, ttMove, stat_bonus(depth, ss));

                // Extra penalty for early quiet moves of the previous ply (~0 Elo)
                if ((ss-1)->moveCount <= 2 && !priorCapture)
                    update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, -stat_bonus(depth + 1, ss));
            }
            // Penalty for a quiet ttMove that fails low (~1 Elo)
            else if (!ttCapture)
            {
                int penalty = -stat_bonus(depth, ss);
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      if (doFullDepthSearch)
      {
          ...
          if (didLMR)
          {
              int bonus = value > alpha ?  stat_bonus(newDepth, ss)
                                        : -stat_bonus(newDepth, ss);
    ...
    */
             ...
             && !priorCapture)
    {
        ...
        update_continuation_histories(ss-1, pos.piece_on(prevSq), prevSq, stat_bonus(depth, ss) * (1 + extraBonus));
    }
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    bonus1 = stat_bonus(depth + 1, ss);
    bonus2 = bestValue > beta + PawnValueMg ? bonus1               // larger bonus
                                            : stat_bonus(depth, ss);   // smaller bonus
