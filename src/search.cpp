...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      {
          doFullDepthSearch = !PvNode || (moveCount > 1 && (!rootNode || bestMove || move != ss->killers[0]));
          didLMR = false;
      }
      ...
      if (PvNode && (moveCount == 1 || (rootNode && !bestMove && move == ss->killers[0]) || (value > alpha && (rootNode || value < beta))))
      {
          (ss+1)->pv = pv;
          (ss+1)->pv[0] = MOVE_NONE;
