...
namespace {
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth, bool killerUpdate = true);
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (!pos.capture_or_promotion(ttMove))
                    update_quiet_stats(pos, ss, ttMove, stat_bonus(depth), depth, false);
...
moves_loop: // When in check, search starts from here
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth, bool killerUpdate) {

    // Update killers
    if (killerUpdate && ss->killers[0] != move)
    {
