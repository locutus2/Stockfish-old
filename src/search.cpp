        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                if (!pos.capture_or_promotion(ttMove))
                    ...
                    update_capture_stats(pos, ss, ttMove, nullptr, 0, stat_bonus(depth));
...
moves_loop: // When in check search starts from here
    ...
    MovePicker mp(pos, ttMove, depth, &thisThread->mainHistory, &thisThread->captureHistory, contHist, countermove, captureCountermove, ss->killers);
  ...
  void update_capture_stats(const Position& pos, Stack* ss, Move move,
      ...
      {
          Square prevSq = to_sq((ss-1)->currentMove);
          pos.this_thread()->captureCounterMoves[pos.piece_on(prevSq)][prevSq] = move;
      }
