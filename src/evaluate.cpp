...
namespace {
  ...
  Score Evaluation<T>::evaluate_pieces() {
    ...
    while ((s = *pl++) != SQ_NONE)
    {
        ...
        b = Pt == BISHOP ? attacks_bb<BISHOP>(s, ~attackedBy[Them][KING] & (pos.pieces() ^ pos.pieces(QUEEN)))
          : Pt ==   ROOK ? attacks_bb<  ROOK>(s, ~attackedBy[Them][KING] & (pos.pieces() ^ pos.pieces(QUEEN) ^ pos.pieces(Us, ROOK)))
