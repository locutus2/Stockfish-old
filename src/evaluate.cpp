...
#undef S
  ...
  Score Evaluation<T>::initiative(Score score) const {
    ...
    Color weakSide = mg < VALUE_ZERO ? WHITE : BLACK;

    int complexity_mg =  complexity_eg
                       - mg_value(kingSafety[weakSide]) / 8
                       + 8;

    // Now apply the bonus: note that we find the attacking side by extracting the
    // sign of the midgame or endgame values, and that we carefully cap the bonus
    // so that the midgame and endgame scores do not change sign after the bonus.
    int u = ((mg > 0) - (mg < 0)) * std::max(std::min(complexity_mg, 0), -abs(mg));
  ...
  Value Evaluation<T>::value() {
    ...
    kingSafety[WHITE] = king<WHITE>();
    kingSafety[BLACK] = king<BLACK>();
