...
namespace {
  ...
  const Score ThreatDoubledPawn   = S( 9, 19);
  ...
  Score evaluate_threats(const Position& pos, const EvalInfo& ei) {
    ...
    // Threat of creation of a doubled pawn
    ...
    if (b)
    {
        b &=  ei.attackedBy[Them][PAWN]
           & ~ei.attackedBy2[Them]
           & ~ei.attackedBy[Us][PAWN]
           & (shift<NORTH>(pos.pieces(Them, PAWN)) | shift<SOUTH>(pos.pieces(Them, PAWN)))
           & (  (pos.pieces(Them, PAWN)  & ei.attackedBy[Us][PAWN])
              | (pos.pieces(Them, KNIGHT, BISHOP) & (ei.attackedBy[Us][KNIGHT] | ei.attackedBy[Us][BISHOP]))
              | (pos.pieces(Them, ROOK)  & ei.attackedBy[Us][ROOK])
              | (pos.pieces(Them, QUEEN) & ei.attackedBy[Us][QUEEN]));

        score += ThreatDoubledPawn * popcount(b);
    }
