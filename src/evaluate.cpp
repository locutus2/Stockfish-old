...
namespace {
  ...
  const Score RookOnPawn          = S( 4, 15);
  const Score RookOnWeakPawn      = S( 4, 16);
        ...
        if (Pt == ROOK)
        {
            ...
            if (relative_rank(Us, s) >= RANK_5)
            {
                ...
                if (alignedPawns)
                {
                    ...
                    // Additional bonus for weak pawns
                    alignedPawns &= ei.attackedBy[Us][PAWN] | ~ei.attackedBy[Them][PAWN] | ei.pinnedPieces[Them];
                    if (alignedPawns)
                        score += RookOnWeakPawn * popcount<Max15>(alignedPawns);
                }
            }
